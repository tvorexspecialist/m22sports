...
namespace Rbofee\Base\Model;

use Rbofee\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExistsFactory;
use Rbofee\Base\Model\Source\NotificationType;
use Magento\Framework\HTTP\Adapter\Curl;
use Magento\Framework\Notification\MessageInterface;
use Magento\Store\Model\ScopeInterface;
use Rbofee\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\Expired;
use Rbofee\Base\Model\AdminNotification\Model\ResourceModel\Inbox\Collection\ExpiredFactory;
...
class Feed
{
    ...
    const XML_LAST_UPDATE = 'rbofee_base/system_value/last_update';

    const XML_FREQUENCY_PATH = 'rbofee_base/notifications/frequency';

    const XML_FIRST_MODULE_RUN = 'rbofee_base/system_value/first_module_run';

    const XML_LAST_REMOVMENT = 'rbofee_base/system_value/remove_date';

    const URL_NEWS = 'rbofee.com/feed-news.xml';//do not use https:// or http

    /**
     * @var array
     */
    private $rbofeeModules = [];
    ...
    public function checkUpdate()
    {
        ...
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            ...
            foreach ($feedXml->channel->item as $item) {
                ...
                if (!$this->validateByRbofeeCount($item->rbofee_module_qty)) {
                    continue;
                }

                if (!$this->validateByNotInstalled((string)$item->rbofee_module_not)) {
                    continue;
                }
     ...
     * @return array|string[]
     */
    private function getInstalledRbofeeExtensions()
    {
        if (!$this->rbofeeModules) {
            ...
            $modules = array_filter(
                $modules,
                function ($item) {
                    return strpos($item, 'Rbofee_') !== false;
                }
            );
            $this->rbofeeModules = $modules;
        }

        return $this->rbofeeModules;
    }
    ...
    private function validateByExtension($extensions, $allModules = false)
    {
        if ($extensions) {
            ...
            if ($extensions) {
                $installedModules = $allModules ? $this->getAllExtensions() : $this->getInstalledRbofeeExtensions();
    ...
    private function validateByNotInstalled($extensions)
    {
        if ($extensions) {
            ...
            if ($extensions) {
                $installedModules = $this->getInstalledRbofeeExtensions();
     ...
     * @return bool
     */
    private function validateByRbofeeCount($counts)
    {
        $result = true;
        ...
        if ($countString) {
            ...
            $rbofeeModules = $this->getInstalledRbofeeExtensions();
            $rbofeeCount = count($rbofeeModules) - 1;//except Rbofee_Base

            if (in_array($rbofeeCount, $counts)
                || ($moreThan && $rbofeeCount >= $moreThan)
            ) {
                $result = true;
            }
        }
