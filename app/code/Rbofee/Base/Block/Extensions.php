...
class Extensions extends \Magento\Config\Block\System\Config\Form\Fieldset
{
    ...
    public function __construct(
        ...
        \Rbofee\Base\Helper\Module $moduleHelper,
        array $data = []
    ) {
    ...
    public function render(AbstractElement $element)
    {
        ...
        foreach ($modules as $moduleName) {
            if (strstr($moduleName, 'Rbofee_') === false
                || $moduleName === 'Rbofee_Base'
    ...
    protected function _getFieldHtml($fieldset, $moduleCode)
    {
        ...
        $moduleName = $this->_replaceRbofeeText($moduleName);
        $status =
             '<a target="_blank">
                <img src="'. $this->getViewFileUrl('Rbofee_Base::images/ok.gif') . '" title="' . __("Installed") . '"/>
             </a>';
        ...
        if ($allExtensions && isset($allExtensions[$moduleCode])) {
            ...
            $url     = $ext['url'];
            $name    = $ext['name'];
            $name = $this->_replaceRbofeeText($name);
            $lastVer = $ext['version'];
            ...
            if (version_compare($currentVer, $lastVer, '<')) {
                $status =
                    '<a href="' . $url . '" target="_blank">
                        <img src="' . $this->getViewFileUrl('Rbofee_Base::images/update.gif') .
        ...
        if ($this->_scopeConfig->getValue('advanced/modules_disable_output/' . $moduleCode)) {
            ...
            $status =
                '<a' . $href . ' target="_blank">
                    <img src="' . $this->getViewFileUrl('Rbofee_Base::images/bad.gif') .
     ...
     * @return mixed
     */
    protected function _replaceRbofeeText($moduleName)
    {
        $moduleName = str_replace('for Magento 2', '', $moduleName);
        $moduleName = str_replace('by Rbofee', '', $moduleName);
